 ...
 *
 * Guards the 1.1.0 review-request line.
 *
 ...
 * (including right after the 1.1.0 upgrade), must go away forever on a click
...
const DAY = 86400;
$configureUrl = 'index.php?controller=AdminModules&configure=megfaq';
...
$unprefixed = array_filter($keys, function ($k) { return strpos($k, 'MEGFAQ_') !== 0; });
ok($unprefixed === [], 'all three carry the MEGFAQ_ prefix (' . implode(', ', $keys) . ')');
ok(MegVentureReviewNudge::REVIEW_URL === 'https://megventure.com/{lang}/testimonials/write?id_product=94',
   'REVIEW_URL targets product id 94 on megventure.com');

echo "\n2) Fresh timestamp: hidden on day 0\n";
Configuration::$store = [];
ok(MegVentureReviewNudge::onInstall(), 'onInstall() succeeds');
$installedAt = (int) Configuration::get('MEGFAQ_REVIEW_INSTALLED_AT');
...
ok((int) Configuration::get('MEGFAQ_REVIEW_DISPLAYS') === 0, 'a hidden page view does not count as a display');

echo "\n3) Quiet period: day 20 hidden, day 21 shown\n";
Configuration::$store['MEGFAQ_REVIEW_INSTALLED_AT'] = (string) (time() - 20 * DAY);
ok(!MegVentureReviewNudge::shouldDisplay(), 'day 20: hidden');
Configuration::$store['MEGFAQ_REVIEW_INSTALLED_AT'] = (string) (time() - 21 * DAY);
ok(MegVentureReviewNudge::shouldDisplay(), 'day 21: shown');
Configuration::$store['MEGFAQ_REVIEW_INSTALLED_AT'] = (string) (time() - 100 * DAY);
...
ok(strpos($html, 'megfaq_review_go=1') !== false, 'render() contains the review link');
ok(strpos($html, 'megfaq_review_dismiss=1') !== false, 'render() contains the dismiss link');
ok(strpos($html, 'megventure.com') === false, 'no direct external URL in the HTML (click routes through the configure page)');
ok((int) Configuration::get('MEGFAQ_REVIEW_DISPLAYS') === 1, 'the view was counted');

echo "\n5) Three unanswered displays, hidden on the fourth\n";
Configuration::$store['MEGFAQ_REVIEW_DISPLAYS'] = '0';
...
ok((int) Configuration::get('MEGFAQ_REVIEW_DISPLAYS') === 3, 'counter stopped at 3');

echo "\n6) A form-POST re-render is shown but not counted\n";
Configuration::$store['MEGFAQ_REVIEW_DISPLAYS'] = '1';
$_POST['submitModuleSettings'] = '1';
ok(MegVentureReviewNudge::render($fake, $configureUrl) !== '', 'still shown while saving settings');
ok((int) Configuration::get('MEGFAQ_REVIEW_DISPLAYS') === 1, 'but the save re-render did not burn a display');
$_POST = [];

echo "\n7) Dismissed: hidden forever\n";
Configuration::$store['MEGFAQ_REVIEW_DISPLAYS'] = '0';
Tools::$values = ['megfaq_review_dismiss' => '1'];
...
ok((int) Configuration::get('MEGFAQ_REVIEW_DISMISSED') === 1, 'dismissed flag written');
ok(!MegVentureReviewNudge::shouldDisplay(), 'hidden right after dismissing');
Configuration::$store['MEGFAQ_REVIEW_INSTALLED_AT'] = (string) (time() - 365 * DAY);
ok(!MegVentureReviewNudge::shouldDisplay(), 'hidden a year later too');

echo "\n8) Review link clicked: recorded, redirected, hidden forever\n";
Configuration::$store = ['MEGFAQ_REVIEW_INSTALLED_AT' => (string) (time() - 30 * DAY)];
Tools::$redirectedTo = null;
Tools::$values = ['megfaq_review_go' => '1'];
MegVentureReviewNudge::handleRequest($fake);
Tools::$values = [];
ok(Tools::$redirectedTo === 'https://megventure.com/tr/testimonials/write?id_product=94',
...
ok(MegVentureReviewNudge::reviewUrl('xx') === 'https://megventure.com/en/testimonials/write?id_product=94',
   'a language megventure.com does not serve falls back to en');

echo "\n9) Upgrade path: no timestamp -> gets one, not shown immediately\n";
require_once dirname(__DIR__) . '/upgrade/upgrade-1.1.0.php';
Configuration::$store = ['MEGFAQ_OTHER' => '1']; // a 1.0.0 shop: no review keys at all
ok(upgrade_module_1_1_0(null) === true, 'upgrade script succeeds');
$stamp = (int) Configuration::get('MEGFAQ_REVIEW_INSTALLED_AT');
...
Configuration::$store['MEGFAQ_REVIEW_INSTALLED_AT'] = '12345';
upgrade_module_1_1_0(null);
ok(Configuration::get('MEGFAQ_REVIEW_INSTALLED_AT') === '12345', 'an existing timestamp is never overwritten');

echo "\n10) onUninstall removes the three keys and touches nothing else\n";
Configuration::$store = [
    'MEGFAQ_REVIEW_INSTALLED_AT' => '12345',
    'MEGFAQ_REVIEW_DISMISSED' => '1',
    'MEGFAQ_REVIEW_DISPLAYS' => '2',
    'MEGFAQ_OTHER' => 'tok',
    'theme' => 'another-modules-value',
    'MEGTESTIMONIAL_WHO2' => 'another-modules-value',
];
MegVentureReviewNudge::onUninstall();
$reviewLeft = array_filter(array_keys(Configuration::$store), function ($k) { return strpos($k, 'MEGFAQ_REVIEW_') === 0; });
ok($reviewLeft === [], 'onUninstall() removed all three review keys');
ok(Configuration::get('MEGFAQ_OTHER') === 'tok', 'the module\'s other keys were left for the module');
ok(Configuration::get('theme') === 'another-modules-value', 'a bare foreign key was not touched');
ok(Configuration::get('MEGTESTIMONIAL_WHO2') === 'another-modules-value', 'another module\'s prefixed key was not touched');

echo "\n11) Wiring: the module file actually calls the nudge\n";
$src = (string) file_get_contents(dirname(__DIR__) . '/megfaq.php');
...
ok(is_file(dirname(__DIR__) . '/upgrade/upgrade-1.1.0.php'), 'upgrade-1.1.0.php ships');
